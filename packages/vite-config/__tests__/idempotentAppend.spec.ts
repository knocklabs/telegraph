...
`;
    ...
    await appendInteractiveBlock(
      "test-files/with-existing-block.css",
      newCssString,
    );
  ...
  it("should handle empty CSS string", async () => {
    ...
    const between = result.substring(
      startIndex + "/* AUTO-GENERATED START */".length,
      endIndex,
    );
...
`;
    ...
    const startCount = (result2.match(/\/\* AUTO-GENERATED START \*\//g) || [])
      .length;
    const endCount = (result2.match(/\/\* AUTO-GENERATED END \*\//g) || [])
      .length;
