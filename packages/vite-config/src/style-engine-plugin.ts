 ...
 */
export const generateInteractiveCss = (
  ...
  const cssVarsByProperty: Record<
    string,
    { cssVar: string; direction?: string }[]
  > = {};
  ...
  Object.entries(cssVarsByProperty).forEach(([baseCssVar, _variants]) => {
    ...
    Object.entries(STATE_MAP).forEach(([state, pseudoSelector]) => {
      ...
      if (potentialKeys.length > 0) {
        ...
        const syntheticVarDeclarations = potentialKeys
          .map((propKey) => {
 ...
 */
export async function appendInteractiveBlock(
  cssFilePath: string,
  css: string,
): Promise<void> {
  try {
...
export function tgphStyleEngine(): Plugin {
  return {
    ...
    async buildStart() {
      ...
      const componentMap = await collectCssVars(
