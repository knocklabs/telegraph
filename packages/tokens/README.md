...
@media (min-width: 768px) {
  .responsive-grid {
    ...
    gap: var(--tgph-space-6);
    padding: var(--tgph-space-6);
  }
}
...
@media (min-width: 1024px) {
  .responsive-grid {
    ...
    gap: var(--tgph-space-8);
    padding: var(--tgph-space-8);
  }
}
```
...
const generateTypes = (tokens) => {
  const keys = Object.keys(tokens).map(key => `"${key}"`).join(' | ');
...
```javascript
// postcss.config.js
const telegraphTokens = require('@telegraph/tokens');
...
module.exports = {
  plugins: [
    require('postcss-custom-properties')({
      importFrom: [
        {
          customProperties: telegraphTokens
        }
      ]
    })
  ]
              ...
              postcssOptions: {
                plugins: [
                  ['postcss-custom-properties', {
                    importFrom: [{ customProperties: telegraphTokens }]
                  }]
                ]
              }
            }
...
// iOS/Swift format
const generateSwift = (tokens) => {
  const swiftTokens = Object.entries(tokens)
    .map(([key, value]) => {
      const name = key.replace(/^--tgph-/, '').replace(/-/g, '_');
      return `static let ${name} = "${value}"`;
    })
    .join('\n');
  
  return `struct TelegraphTokens {\n${swiftTokens}\n}`;
};
...
const generateKotlin = (tokens) => {
  const kotlinTokens = Object.entries(tokens)
    .map(([key, value]) => {
      const name = key.replace(/^--tgph-/, '').replace(/-/g, '_').toUpperCase();
      return `const val ${name} = "${value}"`;
    })
    .join('\n');
  
  return `object TelegraphTokens {\n${kotlinTokens}\n}`;
};

// Generate platform files
fs.writeFileSync('dist/TelegraphTokens.swift', generateSwift(tokens));
fs.writeFileSync('dist/TelegraphTokens.kt', generateKotlin(tokens));
```
...
describe('Telegraph Tokens', () => {
  test('all color tokens have valid hex values', () => {
    const colorTokens = Object.entries(tokens)
      .filter(([key]) => key.includes('blue') || key.includes('gray'));
  ...
  test('spacing tokens use consistent units', () => {
  ...
  test('font size tokens are in ascending order', () => {
    const fontSizes = Object.entries(tokens)
      ...
      .map(([key, value]) => ({
        scale: parseInt(key.split('-').pop()),
        size: parseInt(value.replace('px', ''))
      }))
      .sort((a, b) => a.scale - b.scale);
    
    for (let i = 1; i < fontSizes.length; i++) {
      expect(fontSizes[i].size).toBeGreaterThan(fontSizes[i - 1].size);
    }
  });
});
```
...
describe('Color Contrast', () => {
  test('text colors meet WCAG AA standards', () => {
    const textCombinations = [
      [tokens['--tgph-gray-12'], tokens['--tgph-gray-1']], // Dark text on light bg
      [tokens['--tgph-blue-11'], tokens['--tgph-blue-3']], // Blue text on blue bg
    ];

    textCombinations.forEach(([foreground, background]) => {
      const contrast = chroma.contrast(foreground, background);
      expect(contrast).toBeGreaterThanOrEqual(4.5); // WCAG AA standard
    });
  });

  test('interactive elements meet minimum contrast', () => {
    const interactiveCombinations = [
      [tokens['--tgph-blue-9'], tokens['--tgph-gray-1']], // Primary button
      [tokens['--tgph-gray-6'], tokens['--tgph-gray-1']], // Border
    ];

    interactiveCombinations.forEach(([foreground, background]) => {
      const contrast = chroma.contrast(foreground, background);
      expect(contrast).toBeGreaterThanOrEqual(3); // WCAG AA non-text
    });
  });
});
```
...
describe('Token Visual Tests', () => {
  test('color swatches render correctly', () => {
    ...
    const { container } = render(
      <div>
        <ColorSwatch color={tokens['--tgph-blue-9']} name="blue-9" />
        <ColorSwatch color={tokens['--tgph-red-9']} name="red-9" />
        <ColorSwatch color={tokens['--tgph-green-9']} name="green-9" />
      </div>
    );
...
3. Make changes to token definitions in `src/tokens/`
4. Run generation scripts: `pnpm build`
